...
export default function PaySlip() {
    ...
    const { data, empinfo, month,year } = location.state || {}; // Accessing data safely
    ...
    function fetchEmpData(){
        instance.get(`/api/v1/emp/employee/one/${empinfo.id}`)
        .then((res)=>{
            setEmpData(res.data)
        })
        .catch((err)=>{
    ...
    function translate(){
        ...
        instance.get(`/api/v1/misc/departments/${empData?.empPosition?.departmentId}`)
            .then((res)=>{
        ...
        instance.get(`/api/v1/misc/locations/${empData?.empPosition?.locationId}`)
            .then((res)=>{
    ...
    if(!(department&&designation&&locationName&&empData)){
        return <Loading/>
    }
    return (
        <div className="">
                ...
                <div className="container border-2 border-zinc-600">
                    ...
                    <div className="grid grid-cols-2">
                        <div className="border-t-2 border-r-2 border-zinc-600 p-2">
                            ...
                            <div className="grid grid-cols-2">
                                ...
                                <p className="text-zinc-600 text-sm text-right">{designation}</p>
                            </div>
                            <div className="grid grid-cols-2">
                                <p className="text-zinc-600 text-sm">Department:</p>
                                <p className="text-zinc-600 text-sm text-right">{department}</p>
                            </div>
                            <div className="grid grid-cols-2">
                                <p className="text-zinc-600 text-sm">Location:</p>
                                <p className="text-zinc-600 text-sm text-right">{locationName}</p>
                            </div>
                            <div className="grid grid-cols-2">
                    ...
                    <div className="grid grid-cols-2">
                        <div className="border-t-2 border-r-2 border-zinc-600 ">
                            <div className="grid grid-cols-6 border-b-2 border-zinc-600 p-2">
                                ...
                                <p className="block tracking-wide text-zinc-600 text-md font-semibold ">
                                    Full &#40;<IndianRupee className="inline w-4"/>&#41;
                                </p>
                                <p className="block tracking-wide text-zinc-600 text-md font-semibold">
                                    Actual &#40;<IndianRupee className="inline w-4"/>&#41;
                                </p>
                            </div>
                        ...
                        <div className="border-t-2 border-r-2 border-zinc-600">
                            <div className="grid grid-cols-6 border-b-2 border-zinc-600 p-2">
                                ...
                                <p className="flex items-center tracking-wide text-zinc-600 text-sm font-semibold">
                                <IndianRupee className="h-3.5 inline -m-1.5" />{data.basicSalary+data.hra+data.lta+data.specialAll+data.medicalAll+data.transportAll}
                                </p>
                                <p className="flex items-center tracking-wide text-zinc-600 text-sm font-semibold">
                                <IndianRupee className="h-3.5 inline -m-1.5" />{data.basicSalary+data.hra+data.lta+data.specialAll+data.medicalAll+data.transportAll}
                                </p>
                        ...
                        <div className="border-t-2 border-zinc-600">
                            <div className="grid grid-cols-6 border-b-2 border-zinc-600 p-2">
                                ...
                                <IndianRupee className="h-3.5 inline -m-1.5" />{data.empPf+data.pt+data.tds+data.insurance+data.others}
                                </p>
                            </div>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 p-2">
                        <p className=" text-zinc-600 text-md mb-2">
                            ...
                            <span className="font-bold ml-4"><IndianRupee className="h-3.5 inline -m-1.5" />{
